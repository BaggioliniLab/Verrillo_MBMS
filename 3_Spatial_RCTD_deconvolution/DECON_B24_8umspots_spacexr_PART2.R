...
##### ------------------------------------REF PREP
refdir <- sprintf("./Deconvolution_ref", DS)  # or your path with the exported files
...
# Build path safely and save
stopifnot(exists("myRCTD"))
out_file <- file.path(resultsdir, sprintf("myRCTD_fit%s.RData", DS))
save(myRCTD, file = out_file)
